...
export default function Page() {
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-100 via-slate-200 to-slate-300 dark:from-slate-900 dark:via-darkbg-primary dark:to-slate-950 p-2 md:p-4 gap-4">
      ...
      <div id="main-content" className="flex-1 overflow-hidden flex flex-row gap-2">
        {/* Video Player Panel */}
        <div style={{ width: `${dividerPos}%` }} className="flex-shrink-0 bg-white dark:bg-darkbg-secondary rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
          <VideoPlayer
            video={activeVideo}
        ...
        <div
          ...
          className="w-2 bg-slate-200 dark:bg-slate-700 hover:bg-brand/30 transition-all duration-200 cursor-col-resize group flex items-center justify-center"
        >
          <div className="h-12 w-1 bg-slate-300 dark:bg-slate-600 rounded-full group-hover:bg-brand transition-all duration-200" />
        </div>

        {/* Playlist Panel */}
        <div style={{ width: `${100 - dividerPos}%` }} className="flex-shrink-0 bg-white dark:bg-darkbg-secondary rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
          <PlaylistContainer
